...
class ProfileCubit extends Cubit<ProfileState> {
  ...
  Future<void> addSkill({required String name, required String level}) async {
    ...
    try {
      ...
      final responseData = await profileServices.addUserSkill(name: name, level: lowerLevel);

      if (responseData != null && responseData['success'] == true) {
        // 🚀 سحب المهارات مجدداً بالـ IDs الحقيقية من السيرفر
        userSkills = await profileServices.getUserSkills();

        emit(SkillOperationSuccess(responseData['message'] ?? "Skill added successfully."));
       // _resetToLoaded();
      } else {
        emit(SkillOperationError(responseData?['message'] ?? "Failed to add skill."));
      //  _resetToLoaded();
      }
    } catch (e) {
  ...
  Future<void> deleteSkill({required String skillId}) async {
    emit(SkillOperationLoading());
    try {
      final responseData = await profileServices.deleteUserSkill(skillId: skillId);

      if (responseData != null && responseData['success'] == true) {
        // 🚀 سحب المهارات مجدداً لتحديث الواجهة فوراً بعد الحذف الناجح
        userSkills = await profileServices.getUserSkills();

        emit(SkillOperationSuccess(responseData['message'] ?? "Skill deleted successfully."));
      //  _resetToLoaded();
      } else {
        emit(SkillOperationError(responseData?['message'] ?? "Skill not found."));
        _resetToLoaded();
      }
    } catch (e) {
